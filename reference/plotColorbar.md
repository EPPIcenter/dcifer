...
- by:
  ...
  increment size for tickmark locations. Ignored if `at` is provided.
...
- horiz:
  ...
  a logical value specifying if the colorbar should be drawn
  horizontally.
...
specified outside of this function to control proportions and location
of the colorbar (see examples). To match the colors in the main plot,
`rlim` values should be the same for `plotRel` and `plotColorbar`; if
`rlim = NULL` or `rlim = NA` in `plotRel`, provide the actual range of
relatedness estimates for `plotColorbar` (see examples).

## See also

[`plotRel`](https://eppicenter.github.io/dcifer/reference/plotRel.md)
and [`mixMat`](https://eppicenter.github.io/dcifer/reference/mixMat.md)
...
#> Warning: "isig" is not a graphical parameter
#> Warning: "isig" is not a graphical parameter
#> Warning: "isig" is not a graphical parameter
