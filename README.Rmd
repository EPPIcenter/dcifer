...
if (FALSE) {
pair <- list(dsmp[[ix]], dsmp2[[iy]])
coi  <- c(coi[ix], coi2[iy])
M = 1; confreg = confint; freqlog = TRUE
}
...
To test different hypotheses, we can use `rnull` and `side` arguments. For example, to test whether pairwise relatedness is higher than 0.25 ($H_0: r \leq 0.25$) and obtain one-tailed p-values:

```{r}
dres2 <- ibdDat(dsmp, coi, afreq, rnull = 0.25, side = "right")
```
